...
export default class MainScene extends Phaser.Scene {
  ...
  createGrassBackground() {
    ...
    if (!this.textures.exists(grassKey)) {
      ...
      grassGraphics.generateTexture(grassKey, 64, 64);
      grassGraphics.destroy();
    }

    this.add.tileSprite(0, 0, this.mapWidth, this.mapHeight, grassKey).setOrigin(0, 0);
  }
  ...
  createActors() {
    const playerSheetAvailable = this.textures.exists("player_sprite");
    const npcSheetAvailable = this.textures.exists("npc_sprite");

    const playerTextureKey = playerSheetAvailable
      ? "player_sprite"
      : this.createFallbackTexture("player_fallback", 0x2f6bff, this.playerFrameWidth, this.playerFrameHeight);
    const npcTextureKey = npcSheetAvailable
      ? "npc_sprite"
      : this.createFallbackTexture("npc_fallback", 0xe53935, this.npcFrameWidth, this.npcFrameHeight);
  ...
  createDialogueUI() {
    ...
    this.dialogueUI.setDialogueData([
      {
        name: "小明",
        dialogue: "你好！冒險者！",
  ...
  createSlimePack() {
    ...
    // 測試用：遊戲開始後，將第一隻史萊姆傳送到玩家附近，方便立刻測試戰鬥流程
    if (this.slimes.length > 0) {
      this.slimes[0].setPosition(this.player.x + 80, this.player.y);
    }
